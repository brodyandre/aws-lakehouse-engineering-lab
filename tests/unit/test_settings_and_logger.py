...
class SettingsAndLoggerTestCase(unittest.TestCase):
    def test_settings_exposes_expected_paths_and_buckets(self) -> None:
        ...
        self.assertEqual(settings.serving_data_path.name, "serving")
        self.assertEqual(settings.pipeline_runs_report_path.name, "pipeline_runs")
        self.assertEqual(settings.query_report_path.name, "query")
    ...
    def test_spark_conf_contains_core_defaults(self) -> None:
        ...
        self.assertEqual(
            spark_conf["spark.sql.shuffle.partitions"], str(settings.spark.shuffle_partitions)
        )
        ...
        self.assertIsNone(settings.spark.remote)
